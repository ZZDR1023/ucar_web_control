...
前端 Camera 卡片下新增 `视觉巡线` 面板：

- 支持黑线、白线、黄线；
- 支持调整 ROI 上下边界、线速度、转向增益、最小面积；
- 默认颜色为黄线，因为当前道路地垫更容易先用黄色边线验证；
- 默认 ROI 为画面高度的 `0.45-0.95`，当前摄像头角度下能覆盖地面中下部；
- 默认最小面积为 `250`，适合较细的黄色边线；切换黑线时如果误识别深色地垫或阴影，应调高最小面积或缩窄 ROI；
- 黑线使用 `black_threshold` 识别深色中性区域，默认 `170`。当前摄像头反光会把黑线路面拍成深灰，旧的纯黑阈值会漏检。
...
- 默认只预览，不控制底盘；
- 点击 `启用巡线控制` 后才会写入 `/cmd_vel`；
- 点击 STOP、方向键、导航目标、巡逻任务都会关闭巡线控制。

### 安全限制

- 线速度被限制在 `0.00-0.10 m/s`。
- 转向增益被限制在 `0.00-0.60`，最终角速度限制为 `±0.60 rad/s`。
- 丢线时命令归零，不继续盲走。
- Web 雷达安全框检测到前方障碍时，巡线控制会立即关闭并停车。

### 使用建议

先只使用 `仅更新预览`，观察调试图中的红色 mask 是否稳定覆盖地面线。稳定后再低速启用控制。
